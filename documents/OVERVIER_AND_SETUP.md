...
## 18. Full Source Files

For readers who want to copy-paste and adapt the full implementations, here are all the key files.
...
```javascript
/* This file exposes shared utilities for all actions */
...
async function createOrUpdateFile({
  ...
  const existingFile = checkRes.ok
    ...
    : null;
  const existingSha =
    existingFile && existingFile.sha ? existingFile.sha : undefined;
...
async function createPullRequest(owner, repo, head, base, title, body, token) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Failed to create PR: ${res.status} ${respBody.message || ""}`,
    );
  }
  return res.json();
}
...
### `actions/manage-tokens/index.js`
...
async function main(params) {
  const logger = Core.Logger("manage-tokens", {
  ...
  try {
    ...
    if (!operation) {
      return errorResponse(
        400,
        "missing parameter 'operation'. Valid values: list, get, save, delete",
        logger,
      );
    }
    ...
    if (operation === "get") {
      const missingParams = checkMissingRequestInputs(
        params,
        ["brandName"],
        [],
      );
      ...
      if (!entry || entry.value === undefined) {
        return errorResponse(
          404,
          `Brand '${params.brandName}' not found`,
          logger,
        );
      }
      return {
    ...
    if (operation === "save") {
      ...
      if (typeof tokens === "string") {
        try {
          ...
          return errorResponse(
            400,
            "tokens parameter is not valid JSON",
            logger,
          );
        }
      }
      ...
      const index =
        indexEntry && indexEntry.value ? JSON.parse(indexEntry.value) : [];
      if (!index.includes(safeName)) {
        index.push(safeName);
        await state.put(STATE_INDEX_KEY, JSON.stringify(index), {
          ttl: 31536000,
        });
      }
    ...
    if (operation === "delete") {
      ...
      const index =
        indexEntry && indexEntry.value ? JSON.parse(indexEntry.value) : [];
      const updatedIndex = index.filter((b) => b !== safeName);
      await state.put(STATE_INDEX_KEY, JSON.stringify(updatedIndex), {
        ttl: 31536000,
      });

      return {
        statusCode: 200,
        body: { message: `Brand '${safeName}' deleted`, brandName: safeName },
      };
    }

    return errorResponse(
      400,
      `Unknown operation '${operation}'. Valid values: list, get, save, delete`,
      logger,
    );
  } catch (error) {
    logger.error(error);
...
async function main(params) {
  const logger = Core.Logger("publish-tokens", {
    level: params.LOG_LEVEL || "info",
  });
  ...
  try {
    ...
    if (!entry || entry.value === undefined) {
      return errorResponse(
        404,
        `Brand '${brandName}' not found. Save tokens first.`,
        logger,
      );
    }
    const tokens =
    ...
    // Step 6: Open Pull Request
    const prTitle =
      params.pr_title || `Brand tokens: update CSS variables for ${safeName}`;
    const prBody =
      params.pr_body ||
      `Updates \`${filePath}\` with ${variableCount} CSS custom properties for the **${safeName}** brand.\n\nGenerated by Aramark EDS Token Manager.`;
    ...
    return {
      ...
      body: {
        message: "CSS tokens published successfully",
        brand: safeName,
        css_file: { path: filePath, variables: variableCount, url: fileUrl },
        pull_request: {
          number: pr.number,
          title: pr.title,
          url: pr.html_url,
...
_Thanks for reading! The full source is available at [github.com/skumawat1/appbuilder-poc](https://github.com/skumawat1/appbuilder-poc). If you found this useful, give the repo a ⭐ and feel free to open issues or PRs._

---

**Tags:** `adobe-app-builder` · `adobe-experience-cloud` · `adobe-edge-delivery-services` · `css-design-tokens` · `serverless` · `github-api` · `nodejs` · `javascript`
